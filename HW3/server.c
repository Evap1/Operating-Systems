...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        {
            if ((sizeQueue(vip_waiting_requests) == queue_size) || strcmp(policy, "block") == 0)
            { // as it wassssssss
            ...
            { // drop the request and leaveeeee
                Close(connfd);
                pthread_mutex_unlock(&lock);
                continue;
            }
            else if (strcmp(policy, "dh") == 0)
            { // drop latest request in queue
            ...
            {                                  // NOT A BUSY WAIT
                ...
                {                  // if req A is vip it shouldnt be droped.
