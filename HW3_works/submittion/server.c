...
struct timeval initt, mystarted; // STARTED TO BE DELETED
...
int main(int argc, char *argv[])
{
    ...
    struct timeval arrival; // STARTED TO BE DELETED
    ...
    while (1)
    {
        ...
        if (DEBUG)
        {
            printf("[main] : recieved a task at: %ld, number:%d, Queuesize==%d, TotalSize=%d\n", arrival.tv_sec-initt.tv_sec, connfd, queueSize(waiting_requests),totalReqInQueue());
        }
        ...
        if (totalReqInQueue() >= queue_size)
        {
            ...
            { // as it wassssssss
                ...
                { // waiting for place in wait queue
                    if (DEBUG)
                    {
                        gettimeofday(&mystarted, NULL);
                        printf("[master] : cond waiting at:%ld\n", mystarted.tv_sec-initt.tv_sec) ;
                    }
            ...
            { // drop tail - brand new request recieved
                ...
                {                                // req is regular - current is considered tail
                    ...
                    {
                        gettimeofday(&mystarted, NULL);
                        printf("[master] dt policy! QueueSize=%d, dropped %d at:%ld\n", totalReqInQueue(),connfd ,mystarted.tv_sec-initt.tv_sec);
                    }
                ...
                {                                                     // req is vip - remove from regular
                    ...
                    { // should always succeed since the waiting should not be empty here
                        ...
                        {
                            gettimeofday(&mystarted, NULL);
                            printf("[master] dt policy! QueueSize=%d, dropped %d at:%ld\n",totalReqInQueue()+1,fd_to_delete ,mystarted.tv_sec-initt.tv_sec);
                        }
                        Close(fd_to_delete);
                    }
                }
...
void *workerThread(void *thread_number)
{
    ...
    while (1)
    {
        ...
        { // if empty or vip req, wait
            ...
            {
                gettimeofday(&mystarted, NULL);
                printf("[worker %d] :cond wait at: %ld\n", t_stats->id, mystarted.tv_sec -initt.tv_sec);
            }
