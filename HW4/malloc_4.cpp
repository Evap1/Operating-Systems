...
#define HUGEPAGE_4MB_THRESHOLD 4*1024*1024          // 4MB threshold for HugePages
#define HUGEPAGE_2MB_THRESHOLD 2*1024*1024    // 2MB threshold for scalloc

typedef struct MallocMetaData {         //header stuff
    size_t size;
    bool is_free;
    bool is_hugepage;                   // to support hugepages allocation
    MallocMetaData *next;
    MallocMetaData *prev;
}MetaData;
...
public:
    BlockList() : mmap_blocks(NULL), hugepage_blocks(NULL), is_first_malloc(true), number_used_blocks_sbrk(0), bytes_used_blocks_sbrk(0) {
    ...
    size_t numAllocatedBlocks() {   // need to count the sbrk, mmap and the *not free* blocks
        ...
        current = hugepage_blocks;
        while(current != NULL) {
            counter++;
            current = current->next;
        }

        return counter + this->number_used_blocks_sbrk; // also add the blocks that are “in use” from sbrk (which are kept in the number_used_blocks_sbrk counter).
    }
...
----------------------------------------------------------------------------------------
*                                  Huge Page Support
* for any allocation originally allocated by ‘smalloc, if the allocation request 
* (not including the meta-data) is equal-to or larger than 4𝑀𝐵, allocate a hugepage. 
*
* If the allocation is originally done by means of ‘scalloc’, 
* only do this if the size of one block is larger than 2𝑀𝐵.  
*
* Otherwise, allocate memory as you normally would have (according to part3’s instructions). 
*
----------------------------------------------------------------------------------------
*/
    void* allocateBlock_HugePage(size_t size) {
        void* hugepage_block = mmap(NULL, size + MMD_SIZE, PROT_READ | PROT_WRITE,
                                    MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
        if (hugepage_block == MAP_FAILED){
            return NULL;
        }

        MetaData* new_block = (MetaData*)hugepage_block;
        new_block->size = size;
        new_block->is_free = false;
        new_block->is_hugepage = true;
        new_block->next = NULL;
        new_block->prev = NULL;
        addBlock_HugePage(new_block);
        return hugepage_block;
    }

    void addBlock_HugePage(MetaData* block) {                                   
        if (hugepage_blocks == NULL)
            hugepage_blocks = block;
        else {                                                             
            hugepage_blocks->prev = block;                                     
            block->next = hugepage_blocks;
            hugepage_blocks = block;
        }
    }

   void removeBlock_HugePage(MetaData* block) {                             
        if (block->prev == NULL) {                                          // first in list
            hugepage_blocks = hugepage_blocks->next;
            if (hugepage_blocks)
                hugepage_blocks->prev = NULL;
            block->next = NULL;
        }
        else if (block->next == NULL) {                                     // last in list
            (block->prev)->next = NULL;
    ...
    void freeBlock_HugePage(void* block) {
        MetaData* p = getMetaData(block);
        removeBlock_HugePage(p);
        munmap(p, p->size + MMD_SIZE);                                      // kill it with fire
    }
...
void* smalloc(size_t size) {
    ...
    void* new_block;
    if( size >= HUGEPAGE_4MB_THRESHOLD){
        new_block = block_list.allocateBlock_HugePage(size);
    }
    else if( size >= MAX_BLOCK_SIZE)                               // size too big for sbrk
...
void* scalloc(size_t num, size_t size) {
    ...
    void* new_block;
    if(total_size >= HUGEPAGE_2MB_THRESHOLD){
        new_block = block_list.allocateBlock_HugePage(total_size);
    }
    else {
        new_block = smalloc(total_size);                  // using our lovely assistant smalloc we get a brand new spanking free block
    }
    if (new_block == NULL)
        return NULL;
...
void sfree(void* p) {
    ...
    if (block->is_hugepage){
        block_list.freeBlock_HugePage(p);
    }
...
void* srealloc(void* oldp, size_t size) {
    ...
    if (old_block->is_hugepage && old_block->size < HUGEPAGE_4MB_THRESHOLD){ // 2MB
        if (size == old_block->size)
            return oldp;
        void *new_block = block_list.allocateBlock_HugePage(size);
        if (new_block == NULL)
            return NULL;
    ...
    else if(old_block->size > MAX_BLOCK_SIZE || old_block->is_hugepage) {                      // mmap handling
